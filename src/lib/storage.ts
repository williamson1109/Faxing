...
export async function saveEvent(event: FaxingEvent): Promise<FaxingEvent> { const response = await fetch('/api/events', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: event.id, title: event.name, eventDate: event.date, official: event.official, attendees: event.contestants }) }); if (!response.ok) throw new Error('Unable to save event'); return fromApiEvent(await response.json() as ApiEvent) }
